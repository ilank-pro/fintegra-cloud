...
const CONVEX_URL = process.env.VITE_CONVEX_URL || "https://impressive-snake-259.eu-west-1.convex.cloud";
...
async function seed() {
  ...
  try {
    ...
    console.log(`  ✗ pension-accounts.json: ${err.message?.slice(0, 100)}`);
  }
  ...
  try {
    ...
    console.log(`  ✗ pension-history.json: ${err.message?.slice(0, 100)}`);
  }
